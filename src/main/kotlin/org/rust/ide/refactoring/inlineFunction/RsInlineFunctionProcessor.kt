...
class RsInlineFunctionProcessor(
    ...
    override fun preprocessUsages(refUsages: Ref<Array<UsageInfo>>): Boolean {
        ...
        for (usage in usages) {
            if (removeDefinition && usage is ReferenceUsage) {
                conflicts.putValue(usage.element, "Can't inline function reference")
            }
        }
    ...
    private fun preprocessFunction(originalFunction: RsFunction): RsFunction {
        ...
        replaceReturnToTailExpr(function)
        return function
    }
    ...
    private fun replaceSelfParameter(method: RsFunction) {
        ...
        val newName = "self".freshenName(existingNames)
    ...
    private fun replaceReturnToTailExpr(function: RsFunction) {
...
private class InlineSingleUsage(
    ...
    private fun trySubstituteSimpleLiteral(argument: RsExpr, usages: List<RsElement>): Boolean {
        if (!argument.isSimpleLiteral(usages.size)) return false
    ...
    private fun parseExprAsVariableArgument(argument: RsExpr): VariableArgumentInfo? {
        val (path, argumentType) = when (argument) {
            is RsPathExpr -> argument.path to VariableArgumentType.PLAIN
            is RsUnaryExpr -> {
                val path = (argument.expr as? RsPathExpr)?.path ?: return null
                if (argument.operatorType !in listOf(UnaryOperator.REF, UnaryOperator.REF_MUT)) return null
                path to VariableArgumentType.REF
            }
            else -> return null
        }
    ...
    private fun preprocessFunction(originalFunction: RsFunction): RsFunction {
        ...
        val getFreshName = { name: String ->
            if (name in outerScopeNames) name.freshenName(usedNames) else null
        }
...
private fun RsExpr.isSimpleLiteral(usagesCount: Int): Boolean {
    return when (this) {
        is RsUnaryExpr -> expr?.isSimpleLiteral(usagesCount) == true
        is RsLitExpr -> {
            if (usagesCount == 1) return true
            when (val kind = kind) {
        ...
        is RsBinaryExpr -> usagesCount == 1
            && left.isSimpleLiteral(usagesCount)
            && right?.isSimpleLiteral(usagesCount) == true
        is RsParenExpr -> usagesCount == 1 && expr?.isSimpleLiteral(usagesCount) == true
        else -> false
    }
}
...
private fun String.freshenName(usedNames: MutableSet<String>): String {
    if (this !in usedNames) return this

    val numberSuffix = takeLastWhile { it in '0'..'9' }
    val (name, startIndex) = if (numberSuffix.isEmpty()) {
        this to 1
    } else {
        removeSuffix(numberSuffix) to numberSuffix.toInt()
    }
