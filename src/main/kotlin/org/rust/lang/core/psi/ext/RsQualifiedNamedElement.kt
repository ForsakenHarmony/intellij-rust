...
data class RsQualifiedName private constructor(
    ...
    fun findPsiElement(psiManager: PsiManager, context: RsElement): RsQualifiedNamedElement? {
        ...
        return when (val itemType = itemType) {
            CRATE -> crateRoot
            FIELD, STRUCTFIELD -> resolveAsField(crateRoot)
            else -> resolveAsPsiElement(itemType, crateRoot)
        }
    }

    private fun resolveAsPsiElement(itemType: ItemType, crateRoot: RsFile): RsQualifiedNamedElement? {
        ...
        return when (results.size) {
            0 -> null
            1 -> results.single()
            else -> results.firstOrNull { it.itemType() == itemType }
        }
    }

    private fun resolveAsField(crateRoot: RsFile): RsNamedFieldDecl? {
        val ownerItem = copy(childItems = childItems.dropLast(1))
        val owner = ownerItem.resolveAsPsiElement(ownerItem.itemType, crateRoot) as? RsFieldsOwner ?: return null
    ...
    companion object {
        ...
        private fun RsQualifiedNamedElement.toParentItem(): Item? {
            ...
            if (this is RsMod && isCrateRoot) {
                val crateName = containingCrate?.normName ?: return null
                return Item(crateName, CRATE, this)
            }
            val itemType = parentItemType() ?: error("Unexpected type: `$this`")
        ...
        private fun RsQualifiedNamedElement.itemType(): ItemType? =
            parentItemType() ?: childItemType()
    }
